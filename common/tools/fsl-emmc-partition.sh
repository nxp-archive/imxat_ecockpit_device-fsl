...
options:
  ...
  -q                assume partitions are already set, does not flash system.img and vendor.img
EOF
...
product_raw_file="product_raw.img"
partition_file="partition-table.img"
super_file="super.img"
super_raw_file="super_raw.img"
g_sizes=0
support_dtbo=0
flash_mcu=0
quick=0
...
function erase_partition
{
    ...
    if [ ${num} -gt 0 ] 2>/dev/null; then
        ...
        dd if=/dev/zero of=${node}p${num} bs=1048576 conv=fsync count=$g_sizes
    fi
}
...
function flash_partition
{
    ...
    do
        if [ $? -eq 0 ]; then
            ...
            echo "flash_partition: ${img_name} ---> ${node}p${num}"
            dd if=${image_directory}${img_name} of=${node}p${num} bs=10M conv=fsync
        fi
    done
}
...
if [ "${not_partition}" -ne "1" ] ; then
    ...
    # force re-read partition tables
    blockdev --rereadpt ${node} 
    sleep 3
